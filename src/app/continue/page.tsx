...
import { Loader2, Download, Copy, Upload, Save, Sparkles, FileText, Book, Zap, Target, Eye, Plus, ChevronRight, AlertCircle, CheckCircle2, RefreshCw, Wand2 } from 'lucide-react';
...
export default function ContinuePage() {
  ...
  const continueMenuRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (showExportMenu || showAnalysisMenu || showContinueMenu) {
  ...
  }, [showExportMenu, showAnalysisMenu, showContinueMenu]);
  ...
  const handleAnalysis = async (type: 'original' | 'plot' | 'suggestion') => {
  ...
  const handleContinue = async (type: 'auto' | 'chapter' | 'batch') => {
    if (!originalContent.trim()) {
      alert('请先导入前文内容');
      return;
    }
    ...
    try {
      const response = await fetch('/api/continue', {
        ...
        body: JSON.stringify({
          ...
          characters: chapterOutline || '',
          wordCount: type === 'batch' ? 5000 : 2000,
        }),
      });
      ...
      while (true) {
        ...
        setContinuedContent(fullContent);
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(continuedContent || originalContent);
    alert('已复制到剪贴板');
  };
  ...
  const handleExport = async (format: 'word' | 'txt', content: string) => {
    ...
    const filename = selectedNovelId ? `智能续写作品` : `智能续写内容`;
  ...
  const calculateContentStats = (content: string): ContentStats => {
    ...
    // 计算完读率（简化版算法）
    const density = shuangdianCount / (wordCount / 500 || 1);
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600">专业的AI智能续写工具，提升创作效率</p>
            </div>
            ...
            <Card>
              <div className="flex items-center justify-between border-b border-gray-200/50 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">前文内容</h3>
                </div>
              ...
              <CardBody>
                <Textarea
                  ...
                  rows={15}
                  placeholder="在此粘贴前文内容，或点击「导入文件」上传文档..."
                  fullWidth
            ...
            <Card>
              ...
              <CardBody>
                <Textarea
                  ...
                  rows={5}
                  placeholder="输入本章的剧情梗概，AI会参考这些信息进行续写..."
                  fullWidth
                />
              </CardBody>
            </Card>
                    ...
                    <div ref={analysisMenuRef} className="absolute right-0 z-10 mt-2 w-56 rounded-xl border border-gray-200 bg-white shadow-xl animate-fadeIn">
                      ...
                      >
                        ...
                        <div className="flex flex-col items-start">
                          <span className="font-medium">前文分析</span>
                          <span className="text-xs text-gray-500">分析前文质量</span>
                        </div>
                      </button>
                      <button
                      ...
                      >
                        ...
                        <div className="flex flex-col items-start">
                          <span className="font-medium">剧情建议</span>
                          <span className="text-xs text-gray-500">提供续写建议</span>
                        </div>
                      </button>
                    </div>
                  )}
              ...
              <CardBody className="min-h-[200px]">
                {isAnalyzing ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {analysisResult.completionRatePrediction && (
                      <div>
                        <h4 className="mb-2 font-semibold text-gray-900">完读率预测</h4>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-gradient-to-r from-cyan-500 to-blue-500 transition-all"
                              style={{ width: `${analysisResult.completionRatePrediction}%` }}
                            />
                          </div>
                    ...
                    {analysisResult.plotSuggestions && (
                      <div>
                        <h4 className="mb-2 font-semibold text-gray-900">剧情建议</h4>
                        <ul className="space-y-2">
                          {analysisResult.plotSuggestions.map((suggestion: string, index: number) => (
                            <li key={index} className="flex items-start gap-2 text-sm text-gray-600">
                              <ChevronRight size={16} className="mt-0.5 text-cyan-600 flex-shrink-0" />
                              {suggestion}
                            </li>
            ...
            <Card>
              <div className="flex items-center justify-between border-b border-gray-200/50 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">续写结果</h3>
                </div>
                <div className="flex gap-2">
                  <Button
                    onClick={handleCopy}
                  ...
                  <div className="relative">
                    ...
                    {showExportMenu && (
                      <div ref={exportMenuRef} className="absolute right-0 z-10 mt-2 w-48 rounded-xl border border-gray-200 bg-white shadow-xl animate-fadeIn">
                        <button
                          onClick={() => handleExport('word', continuedContent)}
                        ...
                        <button
                          onClick={() => handleExport('txt', continuedContent)}
                ...
                {isContinuing ? (
                  <div className="flex min-h-[380px] items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600">AI正在续写中，请稍候...</p>
                    </div>
                  </div>
                ) : continuedContent ? (
